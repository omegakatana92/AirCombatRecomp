...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File .\launcher\AirCombatLauncher.ps1
...
#     %APPDATA%\AirCombatRecomp\launcher-config.json (per-user). Falls back
...
[CmdletBinding()]
param(
    [string]$RecompExeName = 'Air_Combat_Recompiled.exe'
)
...
function Get-ConfigDir {
    ...
    $dir = Join-Path $appData 'AirCombatRecomp'
...
$lblInfo.Text = "Select your legally obtained Air Combat disc image (.cue).`r`nChoose OpenBIOS (bundled) or your own legally obtained PS1 BIOS. Click Launch Game to start."
...
$btnBrowse.Add_Click({
    ...
    $dlg.Title = 'Select your legally obtained Air Combat disc image'
...
function Show-Error {
    ...
    [System.Windows.Forms.MessageBox]::Show($form, $Message, 'Air Combat Launcher', 'OK', 'Warning') | Out-Null
}
